...
interface MonthlyFeeJobData {
  type: 'process_all_users' | 'process_user';
  userId?: string;
  billingMonth?: string;
}
 ...
 */
async function processAllUsersMonthlyFees(): Promise<{
  totalUsers: number;
  successfulUsers: number;
  failedUsers: number;
  totalFeesCollected: number;
  totalCardsDeactivated: number;
}> {
  const results: ProcessingResult[] = [];
  
  try {
    // Get all users with active cards
    const { data: usersWithCards, error } = await supabase
      .from('virtual_cards')
      .select('user_id')
      .eq('status', 'active')
    ...
    logger.info(`Processing monthly fees for ${uniqueUserIds.length} users`);
    ...
    for (const userId of uniqueUserIds) {
      try {
        ...
        logger.error(`Failed to process monthly fees for user ${userId}:`, error);
        results.push({
          userId,
          processed: false,
          feesCharged: 0,
    ...
    logger.info('Monthly fee processing completed:', summary);
    return summary;
  } catch (error) {
    logger.error('Failed to process monthly fees:', error);
    throw error;
  }
}
 ...
 */
async function processUserMonthlyFees(userId: string): Promise<ProcessingResult> {
  try {
    // Get user's active cards with monthly fees
    const activeCards = await CardRepository.findByUserId(userId);
    const cardsWithFees = activeCards.filter(
      card => card.status === 'active' && (card.monthly_fee_amount || 0) > 0
    );
    ...
    const totalMonthlyFees = cardsWithFees.reduce(
      (sum, card) => sum + (card.monthly_fee_amount || 0), 
      0
    );
    ...
    if (user.virtual_balance > 0) {
      ...
      await UserRepository.adjustBalance(userId, deductAmount, 'subtract');
    ...
    if (remainingFees > 0) {
      ...
      const sortedCards = [...cardsWithFees].sort(
        (a, b) => (a.remaining_balance || 0) - (b.remaining_balance || 0)
      );
    ...
    logger.error(`Error processing monthly fees for user ${userId}:`, error);
    throw error;
  }
}
 ...
 */
export function createMonthlyFeeWorker(): Worker {
  ...
  const worker = new Worker(
    ...
    async (job: Job<MonthlyFeeJobData>) => {
      const { type, userId } = job.data;

      logger.info(`Processing monthly fee job: ${type}`, { userId });
      ...
      try {
        ...
        logger.error('Monthly fee processing failed:', error);
  ...
  worker.on('completed', (job) => {
    logger.info(`Monthly fee job ${job.id} completed`, job.returnvalue);
  });

  worker.on('failed', (job, err) => {
    logger.error(`Monthly fee job ${job?.id} failed:`, err);
  });
