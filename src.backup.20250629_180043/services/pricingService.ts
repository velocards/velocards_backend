...
class PricingService {
   ...
   */
  async getUpcomingRenewal(userId: string): Promise<{
    ...
    try {
      ...
      logger.error('Error getting upcoming renewal info:', error);
      throw error;
    }
  }
   ...
   */
  async getMonthlyFeeBreakdown(userId: string): Promise<{
    currentMonth: {
      paid: number;
      pending: number;
      failed: number;
    ...
    try {
      ...
      logger.error('Error getting monthly fee breakdown:', error);
      throw error;
    }
  }
}
