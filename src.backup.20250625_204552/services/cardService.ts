...
import { cardConfig, defaultUserData } from '../config/env';
  ...
  static async createCard(
    ...
    try {
      ...
      if (!programs || programs.length === 0) {
        throw new ValidationError('No card programs available');
      }
      
      const selectedProgram = programs[0]!; // We checked length above
      ...
      // Validate user profile completeness if required
      if (cardConfig.requireCompleteProfile) {
        if (!user.metadata?.['first_name'] || !user.metadata?.['last_name'] || 
            !user.metadata?.['address'] || !user.metadata?.['city'] || 
            !user.metadata?.['state'] || !user.metadata?.['zip'] || !user.phone) {
          throw new ValidationError('Complete user profile required for card creation. Please update your profile.');
        }
      }
      ...
      try {
        const cardRequest: CreateCardRequest = {
          ...
          Limit: spendingLimit,
          FirstName: user.metadata?.['first_name'] || defaultUserData.firstName,
          LastName: user.metadata?.['last_name'] || defaultUserData.lastName,
          Address1: user.metadata?.['address'] || defaultUserData.address,
          City: user.metadata?.['city'] || defaultUserData.city,
          State: user.metadata?.['state'] || defaultUserData.state,
          Zip: user.metadata?.['zip'] || defaultUserData.zip,
          CountryIso: user.metadata?.['country'] || defaultUserData.country,
          ExpMonth: expMonth,
          ExpYear: expYear,
          PhoneNumber: user.phone || defaultUserData.phone
        };
