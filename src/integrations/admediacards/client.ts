...
export interface AdmediacardsShowPAN {
  CardID?: number;
  CcNum: string;    // Real API field name for card number
  Cvx2: string;     // Real API field name for CVV
  ExpMonth: string;
  ExpYear: string;
  // Keep legacy fields for mock compatibility
  PAN?: string;
  CVV?: string;
}
  ...
  async showPAN(cardId: number): Promise<AdmediacardsShowPAN> {
    if (this.isLive) {
      try {
        ...
        logger.error('Failed to get Admediacards card PAN', { error, cardId });
        throw error;
      }
    }
    ...
    if (!card) {
      logger.info('Creating mock card data for existing card', { cardId });
