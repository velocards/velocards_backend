...
export class TierController {
   ...
   */
  static async getUpcomingRenewal(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user!.sub;
      const renewalInfo = await pricingService.getUpcomingRenewal(userId);
      sendSuccess(res, renewalInfo);
    } catch (error) {
      next(error);
    }
  }
   ...
   */
  static async getMonthlyFeeBreakdown(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user!.sub;
      const breakdown = await pricingService.getMonthlyFeeBreakdown(userId);
