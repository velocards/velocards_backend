...
import { getSumsubClient, SumsubApplicantData } from '../integrations/sumsub/client';
...
export class KYCService {
  ...
  static async initiateKYC(userId: string): Promise<KYCInitiateResponse> {
    try {
      ...
      let isResume = false;
      let applicant: SumsubApplicantData;

      // Check if user has an existing applicant ID (resuming KYC)
      if (user.kyc_applicant_id && user.kyc_status === 'pending') {
        try {
          // Try to get existing applicant
          applicant = await sumsubClient.getApplicantStatus(user.kyc_applicant_id);
          // getApplicantStatus might not return the ID, so add it from the user record
          if (!applicant.id) {
            applicant.id = user.kyc_applicant_id!;
          }
          isResume = true;
          logger.info('Resuming existing KYC session', {
            userId,
            applicantId: applicant.id,
            currentStatus: applicant.review?.reviewStatus,
          });
        } catch (error: any) {
          logger.warn('Failed to get existing applicant, creating new one', {
            userId,
            existingApplicantId: user.kyc_applicant_id,
            error: error.message,
          });
          // If we can't get the existing applicant, create a new one
          applicant = await sumsubClient.createApplicant(
            userId,
            user.email,
        ...
        try {
          ...
          if (error.response?.status === 409 && error.response?.data?.description?.includes('already exists')) {
            logger.warn('Applicant already exists in Sumsub, attempting to retrieve', {
              userId,
              error: error.response?.data
            });
            
            // Extract applicant ID from error message if available
            const match = error.response.data.description.match(/already exists: (\w+)/);
            if (match && match[1]) {
              const existingApplicantId = match[1];
              logger.info('Found existing applicant ID from error', {
                userId,
                applicantId: existingApplicantId
              });
              
              try {
                // Try to get the existing applicant
                applicant = await sumsubClient.getApplicantStatus(existingApplicantId);
                if (!applicant.id) {
                  applicant.id = existingApplicantId;
                }
                
                // Save the applicant ID to prevent this issue in the future
                await UserRepository.updateKYCStatus(userId, 'pending', {
                  kyc_applicant_id: existingApplicantId,
                });
                
                logger.info('Retrieved existing applicant and saved ID', {
                  userId,
                  applicantId: existingApplicantId
                });
              } catch (retrieveError: any) {
                logger.error('Failed to retrieve existing applicant', {
                  userId,
                  applicantId: existingApplicantId,
              ...
              throw error; // Re-throw if we can't extract applicant ID
            }
          } else {
            throw error; // Re-throw if not a 409 conflict
          }
        }
      }
      ...
      // Update user's KYC status and applicant ID if needed
      if (!user.kyc_applicant_id || user.kyc_applicant_id !== applicant.id) {
        await UserRepository.updateKYCStatus(userId, 'pending', {
  ...
  static async getKYCStatus(userId: string): Promise<KYCStatusResponse> {
    try {
      ...
      const sumsubClient = getSumsubClient();
      const applicant = await sumsubClient.getApplicantStatus(user.kyc_applicant_id);
  ...
  static async resetKYC(userId: string): Promise<void> {
    try {
      ...
      const sumsubClient = getSumsubClient();
      await sumsubClient.resetApplicant(user.kyc_applicant_id);
      ...
      // Check if it's a specific Sumsub error
      if (error.response?.status === 400) {
        throw new AppError('KYC_CANNOT_RESET', 'KYC verification cannot be reset in current state', 400);
      } else if (error.response?.status === 404) {
        throw new AppError('KYC_NOT_FOUND', 'KYC verification not found', 404);
      }
