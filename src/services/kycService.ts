...
export class KYCService {
  ...
  static async initiateKYC(userId: string): Promise<KYCInitiateResponse> {
    try {
      ...
      if (user.metadata?.['kyc_applicant_id'] && user.kyc_status === 'pending') {
        try {
          // Try to get existing applicant
          applicant = await sumsubClient.getApplicantStatus(user.metadata['kyc_applicant_id']);
          // getApplicantStatus might not return the ID, so add it from metadata
          if (!applicant.id) {
            applicant.id = user.metadata['kyc_applicant_id'];
          }
          isResume = true;
          ...
          logger.warn('Failed to get existing applicant, creating new one', {
            userId,
            existingApplicantId: user.metadata['kyc_applicant_id'],
            error: error.message,
          });
      ...
      if (!user.metadata?.['kyc_applicant_id'] || user.metadata['kyc_applicant_id'] !== applicant.id) {
