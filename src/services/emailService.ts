...
export class EmailService {
  ...
  static isReady(): boolean {
    return emailProvider.getActiveProvider() !== 'Console';
  }
  ...
  static async sendInvoiceEmail(invoiceId: string): Promise<void> {
    try {
      if (!this.isReady()) {
        logger.warn('Email service not initialized, skipping email send', { invoiceId });
        return;
      }
   ...
   */
  private static async getInvoiceEmailData(invoiceId: string): Promise<InvoiceEmailData> {
    // Get invoice
   ...
   */
  private static generateInvoiceEmailTemplate(data: InvoiceEmailData): EmailTemplate {
...
This is an automated email. Please do not reply to this message.
  ...
  private static async sendEmailInternal(options: {
    ...
    const fromEmail = env.INVOICE_FROM_EMAIL || 'invoices@velocards.com';
    const fromName = env.INVOICE_FROM_NAME || 'VeloCards Finance';
