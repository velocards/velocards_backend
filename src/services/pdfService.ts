...
export class PDFService {
  ...
  static async generateInvoicePDF(invoiceId: string): Promise<string> {
    try {
      logger.info('Starting PDF generation', { invoiceId });
      ...
      logger.info('PDF generated successfully', { invoiceId, pdfUrl });
      return pdfUrl;
    } catch (error) {
      logger.error('Failed to generate PDF', { invoiceId, error });
      throw new AppError('PDF_GENERATION_FAILED', 'Failed to generate invoice PDF', 500);
    }
  }
   ...
   */
  private static async getInvoiceData(invoiceId: string): Promise<{
    invoice: InvoiceData;
    items: InvoiceItem[];
    user: UserProfile;
  }> {
    ...
    if (itemsError) {
      throw new AppError('DATABASE_ERROR', 'Failed to fetch invoice items', 500);
    }
   ...
   */
  private static generateInvoiceHTML(data: {
    invoice: InvoiceData;
    items: InvoiceItem[];
    user: UserProfile;
  }): string {
   ...
   */
  private static async uploadPDFToStorage(invoiceId: string, pdfBuffer: Buffer): Promise<string> {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('bucket') || error.message.includes('not found')) {
          throw new AppError('STORAGE_BUCKET_MISSING', 
            'Storage bucket not configured. Please create "documents" bucket in Supabase Storage.', 
            500);
        }
        throw new AppError('STORAGE_ERROR', `Failed to upload PDF: ${error.message}`, 500);
      }
      ...
      logger.error('PDF storage upload failed', { invoiceId, error });
      
      // Fallback: return a placeholder URL for development
      if (process.env['NODE_ENV'] === 'development') {
        logger.warn('Using placeholder PDF URL in development mode');
   ...
   */
  static async downloadInvoicePDF(invoiceId: string, userId: string): Promise<{
    buffer: Buffer;
    filename: string;
  }> {
    ...
    if (!invoice.pdf_url) {
      ...
      if (!updatedInvoice?.pdf_url) {
        throw new AppError('PDF_GENERATION_FAILED', 'Failed to generate PDF', 500);
      }
    ...
    if (downloadError || !data) {
      throw new AppError('DOWNLOAD_ERROR', 'Failed to download PDF', 500);
    }
